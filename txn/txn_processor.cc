...
void TxnProcessor::RunOCCScheduler() {
  ...
  while (tp_.Active()) {
    ...
    if (txn_requests_.Pop(&txn)) {
      ...
      tp_.RunTask(  // Run the transaction in a new thread.
        new Method<TxnProcessor, void, Txn*>(
    ...
    while (completed_txns_.Pop(&txn)) {
      ...
      if (txn->Status() == COMPLETED_C) {
        ...
        // Check for readset conflicts.
        for (set<Key>::iterator itr = txn->readset_.begin();
              itr != txn->readset_.end(); ++itr) {
  ...
  while (tp_.Active()) {
    ...
    for (i = 0; i < n; i++) {
      if (completed_txns_.Pop(&txn)) {
        ...
        tp_.RunTask(  // Run the validation in a new thread.
          new Method<TxnProcessor, void, Txn*, set<Txn*> >(
            this, &TxnProcessor::ValidateTxnParallel, txn, activeSetCopy));
      } else {
    ...
    for (i = 0; i < m; i++) {
      if (validated_txns_.Pop(&txn)) {
        ...
        if (txn->is_valid_) {  // If transaction is valid, mark as committed.
          ...
          txn->reads_.clear();
          txn->writes_.clear();
          txn_requests_.Push(txn);  // Transaction will re-start next time we
                                    // check for a new transaction request.
        }
      } else {
...
void TxnProcessor::ValidateTxnParallel(Txn* txn, set<Txn*> activeSet) {
  ...
  if (txn->Status() == COMPLETED_C) {
    ...
    if (!validationFailure) {
      for (set<Txn*>::iterator t = activeSet.begin();
            t != activeSet.end() && !validationFailure; ++t) {
        // Check for conflicts between txn's writeset and t's readset
        // or writeset.
        for (set<Key>::iterator itr = txn->writeset_.begin();
              itr != txn->writeset_.end(); ++itr) {
          if ((*t)->readset_.find(*itr) != (*t)->readset_.end() ||
              (*t)->writeset_.find(*itr) != (*t)->writeset_.end()) {
            validationFailure = true;  // A conflict was found.
            break;
          }
        }
      }
    ...
    validated_txns_.Push(txn);  // Push the transaction onto our list of those
                                // to be validated.
  } else if (txn->Status() == COMPLETED_A) {
    txn->status_ = ABORTED;
  } else {
